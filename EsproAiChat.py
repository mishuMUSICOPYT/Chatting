...
async def start_command(_, m: t.Message):
    ...
    await m.reply_photo(
        ...
        caption=(
            ...
            "✴️ For help or updates, contact the owner or add me to a group using the button below."
        ),
...
async def chatbots(_, m: t.Message):
    ...
    try:
        ...
        await m.reply_text("âŒ Failed to process message.")
        print(f"Error: {e}")
...
async def smart_chat(_, m: t.Message):
    ...
    try:
        ...
        await m.reply_text("âŒ Error. Try again later.")
        print(f"Error: {e}")
